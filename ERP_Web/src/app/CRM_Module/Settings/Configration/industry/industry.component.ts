...
export class IndustryComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private http: BackendService, private router: Router, private activatedRoute: ActivatedRoute) {
    this.activatedRoute.queryParamMap.subscribe((params) => {
      this.industryId = params.get('industryid'); // Updated parameter name
    });
  ...
  getapi(): void {
    this.http.getapi('api/Common/GetIndustry').subscribe((res) => { // Updated API endpoint
  ...
  getIndustryById() {
    this.http.getapi('api/Common/GetIndustriesby/' + this.currentIndustryId).subscribe((res) => { // Updated API endpoint
      console.log(res);
      debugger;
      this.industryForm.get("industryId")?.setValue(res.data.industryId); // Updated field name
      this.industryForm.get("description")?.setValue(res.data.description);
    });
  }

  deleteIndustry(id: number): void {
    this.http.deleteapi(`api/Common/DeleteIndustry/${id}`).subscribe(() => { // Updated API endpoint
